...
Версия 2.0 оставляет только один защищённый путь управления XKeen: Companion по HTTPS с точным pin сертификата и отдельным отзываемым токеном каждого телефона. Старый самостоятельный HTTP-сервис удалён.
...
2. Создайте профиль роутера в KeenWG.
3. Откройте «Система» → «Установить или обновить Companion».
4. Сверьте SSH fingerprint с доверенным источником. Пароль не отправляется до вашего подтверждения.
5. Проверьте read-only отчёт и отдельно подтвердите установку или привязку телефона.
6. Настройте только нужные модули: RCI для WireGuard, Collector для истории, источники подключений для XKeen/VLESS.

Если Companion 2.0 уже установлен, мастер выполняет только pairing: бинарник и конфигурация на роутере не перезаписываются. Обновление с 1.0 сохраняет identity, доверенные устройства, активный маршрут, подписки и пользовательские правила.
...
Не публикуйте subscription URL, VLESS URI, WireGuard private key, SSH-пароль, pairing secret, device token, backup, hostname, MAC или полный публичный IP. Для отчёта используйте только встроенный санитизированный экспорт.
